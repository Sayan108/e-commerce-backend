...
class CartModel {
  ...
  /* -------------------- SHARED SELECT -------------------- */
  static get CART_FIELDS() {
    return "_id userId productId thumbnail itemname quantity price";
  }
  ...
  static async getCartByUserId(userId) {
    if (cfg.db.type === dbs.MONGODB) {
      return CartM.find({ userId }).select(this.CART_FIELDS).lean();
    }
  ...
  static async deleteCartItem(id) {
    if (cfg.db.type === dbs.MONGODB) {
      return CartM.findByIdAndDelete(id)?.select(this.CART_FIELDS)?.lean();
    }
